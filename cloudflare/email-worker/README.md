...
If RemoteLab also needs to reach arbitrary external recipients, configure a local outbound provider in `~/.config/remotelab/agent-mailbox/outbound.json` such as `resend_api` for a custom-domain sender or `apple_mail` for local fallback testing; Cloudflare's platform-level verified-destination rule itself cannot be disabled.
