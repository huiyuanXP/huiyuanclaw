...
async function runSessionBoardLayoutSuggestion(sessionMeta, _options = {}) {
  ...
  const prompt = [
    ...
    '{',
    '  "columns": [',
    '    { "key": "column_a", "label": "Column A", "order": 10, "description": "Optional short description" }',
    '  ],',
    '  "placements": [',
    '    { "sessionId": "exact-session-id", "columnKey": "column_a", "order": 10, "priority": "high", "reason": "short reason" }',
    '  ]',
    '}',
    ...
    '- Treat the example keys and labels as placeholders only, not suggested board copy.',
...
async function runSessionTaskBoardSuggestion(sessionMeta, _options = {}) {
  ...
  const prompt = [
    ...
    '  ],',
    '  "assignments": [',
    '    { "sessionId": "exact-session-id", "taskId": "task_alpha" }',
    '  ],',
    '  "placements": [',
    '    { "taskId": "task_alpha", "columnKey": "column_a", "order": 10, "priority": "high", "reason": "short reason" }',
    '  ]',
    '}',
    'Rules for JSON output:',
    '- Use exact session ids from the provided metadata.',
    ...
    '- Use only "high", "medium", or "low" for priority.',
    '- Use integer order values; lower order means earlier from left to right for columns and top to bottom for cards.',
    '- Treat the example keys, ids, and labels as placeholders only, not suggested board copy.',
