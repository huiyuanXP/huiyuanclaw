...
export function buildBuiltinEvomapRecipeProfile(profileId = 'hotel-housekeeping-analysis', options = {}) {
  ...
  const gene = {
    ...
    validation: [
      'Verify recalculated room totals reconcile with the source workload ledger.',
      'Verify incentive totals explain any gap between attendance-adjusted baseline and final payout.',
      'Verify linen and amenity costs can be expressed as per-room metrics for the same period.',
      'Verify the final report surfaces missing inputs and unusual values explicitly instead of imputing silently.',
    ],
    epigenetic_marks: [],
